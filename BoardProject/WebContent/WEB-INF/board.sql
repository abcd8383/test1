...
(select b.*, nvl(c.comment_count,0) 
from board b, (select bno, count(*) as comment_count from board_comment group by bno) c
where b.bno = c.bno(+) order by b.bno desc) x 
where rownum<=6;

--step3 1페이지 : 1~7, 2페이지 8~14, 3페이지 15~21
--페이지번호 n, 페이지당 보여줄 글목록 갯수 x
--시작 글 번호 : (n-1)*x+1 / 끝 글 번호 : n*x
select * from (select rownum as rn, x.* from
(select b.*, nvl(c.comment_count,0) 
from board b, (select bno, count(*) as comment_count from board_comment group by bno) c
where b.bno = c.bno(+) order by b.bno desc) x 
)where rn between 8 and 14; 

--페이지번호를 구해서 where 간단하게
select*from (select ceil(rownum / 7) as pagenum, bno, title, bdate, bcount, writer, content, blike, bhate from
(select b.*, nvl(c.comment_count,0) 
from board b, (select bno, count(*) as comment_count from board_comment group by bno) c
